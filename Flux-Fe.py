...
plt.close("all")
data=np.genfromtxt('Cs-137_Fe.txt', skip_header = 5, skip_footer = 1, autostrip = True)
...
plt.figure(10)
plt.scatter(x, 1/(2.2e30*0.0267*thickness)*(fluxlist/fluxin), label = "under gaussian")

#%%
fluxin = np.sum(currentData)
print(fluxin)
plt.scatter(x, 1/(2.2e30*0.0267*thickness)*(totalcounts/fluxin), label = "all data")
plt.legend()
...
plt.legend()
ax = plt.gca()
ax.set_ylim([-0.1*10e-59, 0.2e-28])
plt.show()
#%%
